...
def descriptive_statistics(col):
    ...
    if (col.dtype == np.object) or (col.dtype == 'category'):
...
def QT_Transformer(data, input_cols,categorical_cols, numerical_cols, replace = False):
    quantile = QuantileTransformer(output_distribution='normal')
    data = data.reset_index(drop = True)
    add_columns = []
    if replace:
        data.loc[:,input_cols] = quantile.fit_transform(data[input_cols])
    else:
        temp_df = pd.DataFrame(quantile.fit_transform(data[input_cols]))
        temp_df.columns = [c + '_QT' for c in input_cols]
        add_columns.extend([c for c in temp_df.columns])
...
def handing_missing_values_for_cat(data, categorical_cols):
...
def min_max_Scaler(data, numerical_cols):
    ...
    data.loc[:,numerical_cols] = MinMaxScaler().fit_transform(data[numerical_cols])
...
def dataprocess(data, excluding_feature, data_split_by_time = None):
    ...
    print('##### QT_Transformer  #####')

    data,categorical_cols,numerical_cols = QT_Transformer(data,
                                                          input_cols = numerical_cols,
                                                          categorical_cols = categorical_cols,
                                                          numerical_cols = numerical_cols,
                                                          replace = True)
    ...
    data = min_max_Scaler(data, numerical_cols)

    print('##### handing_missing_values_for_cat  #####')
    data = handing_missing_values_for_cat(data, categorical_cols)
    
    print('##### saving dataset  #####')
    data["Wait_Time"] = np.log1p(data["Wait_Time"])
    data.to_parquet('../dataset/dataset_1001_1751.parquet', engine='fastparquet',index = False)
...
def tabnet_preprocess(data, excluding_feature, config):
    ...
    categorical_features = data_info[(data_info['feature_dtype'] == np.object) | (data_info['feature_dtype'] == 'category')]
    ...
    target_values = data['Wait_Time'] 
    colnames = []
    corr_values = []
    for col in tqdm(numerical_features['feature_name'].tolist()):
        # col = numerical_features['feature_name'].tolist()[0]
        corr = abs(np.corrcoef(target_values, data[col])[0,1])
        colnames.append(col)
        corr_values.append(corr)
    corr_table = pd.DataFrame({'col':colnames, 'corr_values':corr_values})
    corr_table = corr_table.sort_values(by = 'corr_values', ascending = False)

    select_cols = corr_table.head(500)['col'].tolist()

    features = select_cols + categorical_features['feature_name'].tolist()

    # handling categorical columns 
    categorical_col = {}
    categorical_encoder = {}
    for col in tqdm(categorical_features['feature_name']):
    ...
    emb_dim_biner = [0,2,4,7,11]
    cat_emb_dim = pd.cut(np.array(cat_dims), emb_dim_biner, labels=[1,2,5,8]).tolist()
    ...
    for col in tqdm(colnames):
        ...
        if missing_cnt > 0:
            data[col] = data[col].fillna(0)
    # data["Wait_Time"] = np.log1p(data["Wait_Time"])
    return data, features, config,corr_table
...
if __name__ == '__main__':
    ...
    # data_summary = other_data.apply(descriptive_statistics, axis = 0, result_type='expand').T

...
class DataLoader(object):
    ...
    def transform(self, df, index_col, date_col, target_col, feature_col=None, exculding_cols=None):
        ...
        assert dtype_table[index_col] == np.dtype(
            'object'), 'Please {} must convert into object '.format(index_col)
        assert dtype_table[date_col] == np.dtype(
            'datetime64[ns]'), 'Please {} must convert into datetime64[ns] '.format(date_col)
        ...
        if exculding_cols is not None:
            self.exculding_cols = [index_col,
                                   date_col, target_col] + exculding_cols
        else:
        ...
        if feature_col is not None:
            ...
            self.feature_col = [
                c for c in df.columns if c not in self.exculding_cols]
    ...
    def excluding_abnormal_data(self, data_type, method, use_feature, **kwargs):
        ...
        self.kwargs = kwargs
        if data_type == 'all':
            self.df = self._abnormal_detection(
                self.df, self.method, self.use_feature, **self.kwargs)
            return self.df
    ...
    def _abnormal_detection(df, method, use_feature, **kwargs):
        if method == 'rule_based':
            ...
            assert 'Wait_Time_threshold' in kwargs.keys(
            ), 'Please, importing Wait_Time_threshold'

            df = df[~((df['Z03_WAITTIME_cnt'] == 0) & (
                df['Wait_Time'] > kwargs['Wait_Time_threshold']))]
    ...
    def training_data(self):
        ...
        if self.test_split_date is not None:
            input_df = self.df[self.df[self.date_col]
                               < self.test_split_date].copy()
        else:
    ...
    def testing_data(self):
        ...
        if self.test_split_date is not None:
            test_df = self.df[self.df[self.date_col]
                              >= self.test_split_date].copy()
        else:
            return None

        test_y = test_df[self.target_col]
        # .drop(columns = [self.index_col, self.date_col, self.target_col])
        test_x = test_df
    ...
    def exporting_data(self, df, split_set):
        for i, train_id, valid_id in split_set:
            ...
            if self.data_type == 'train_dataset':
                train_df = self._abnormal_detection(
                    train_df, self.method, self.use_feature, **self.kwargs)
    ...
    def all_data(self):
        return self.df["Serial_Number"], self.df[self.feature_col]
...
if __name__ == '__main__':
    ...
    selected_col = [c for c in D3.columns if c.startswith(
        'Z03') or c == 'Serial_Number']
    df = df.merge(D3[selected_col], how='left', on='Serial_Number')

    selected_col = [c for c in D4.columns if c.startswith(
        'Z03') or c == 'Serial_Number']
    df = df.merge(D4[selected_col], how='left', on='Serial_Number')

    excluding_feature = ['Serial_Number',
                         'Queue_Time', 'Wait_Time', 'Meal_Time']
    features = [c for c in df.columns if c not in excluding_feature]
    dataloader = DataLoader(k_fold=1, split_size=0.1,
                            valid_split_date='2017-07-01 00:00:00', test_split_date='2017-10-01 00:00:00')
    dataloader.transform(df, index_col='Serial_Number',
                         date_col='Queue_Time',
                         target_col='Wait_Time',
                         feature_col=features,
                         exculding_cols=excluding_feature)
    ddf = dataloader.excluding_abnormal_data(
        data_type='all', method='rule_based', use_feature=None, Wait_Time_threshold=20)
    ddf = dataloader.excluding_abnormal_data(data_type='all', method='isolation_forest',
                                             use_feature=[
                                                 'Wait_Time', 'Z03_WAITTIME_group_size', 'Z03_WAITTIME_cnt'],
                                             contamination=0.01)
